...
/**
 * CollectionImport Component
 * Handles collection import modal with file upload, validation, and background job processing
 */
const CollectionImport = ({
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      const blob = await collectionApi.getImportTemplate();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'collections-import-template.csv';
  ...
  const handleValidateImport = useCallback(async () => {
    ...
    try {
      ...
      const response = await collectionApi.validateCollectionsImport(importFile);
      if (response.success) {
          ...
          if (status.status === 'completed') {
            ...
            if (attempts < maxAttempts && !isCancelled && currentJobIdRef.current === jobId) {
              ...
              if (attempts >= maxAttempts) {
                toast.error('Import is taking longer than expected. Please check the status later.');
              }
              if (pollInterval) clearTimeout(pollInterval);
              isCancelled = true;
            }
  ...
  const handleImportCollections = useCallback(async (useAsync = false) => {
    if (!importFile) {
      toast.error('Please select a file');
      return;
    }
    ...
    try {
      ...
      const response = await collectionApi.importCollections(importFile, { async: useAsync });
      
      if (response.success) {
        if (useAsync && response.data?.jobId) {
          ...
          toast.success(`Import completed: ${response.data.success} succeeded, ${response.data.failed} failed`);
      ...
      toast.error(error.response?.data?.message || 'Failed to import collections');
    } finally {
  ...
  return (
    <ErrorBoundary
      title="Import Error"
      message="An error occurred while importing collections. Please try again."
      showReload={false}
    >
      <div
        ref={importModalRef}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <input
              ...
              aria-describedby="file-help-text file-error-text"
              aria-invalid={!!fileSizeError}
            />
            <p id="file-help-text" className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              Upload a CSV or Excel file with collection data (max 10MB). Format: Location Type, Location Name, Address, Collection Items (materialType:weightkg@₹rate/kg;...), etc.
            </p>
            {fileSizeError && (
              <p id="file-error-text" className="text-xs text-red-600 dark:text-red-400 mt-1 flex items-center gap-1" role="alert">
                ...
                {importPreview.summary && (
                  <div className="mt-3 pt-3 border-t border-primary-200 dark:border-primary-800">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-gray-900 dark:text-white">{importPreview.summary.totalCollections}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600 dark:text-gray-400">Total Weight:</span>
                      <span className="font-medium text-gray-900 dark:text-white">{importPreview.summary.totalWeight} kg</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600 dark:text-gray-400">Total Amount:</span>
                      <span className="font-medium text-gray-900 dark:text-white">{formatCurrency(importPreview.summary.totalAmount, { maximumFractionDigits: 2 })}</span>
                    </div>
                  </div>
                )}
                {importPreview.preview && importPreview.preview.some(p => !p.valid) && (
                  <div className="mt-3">
                    <Button
                      ...
                      aria-controls="preview-errors-list"
              ...
              {importResults.results && importResults.results.length > 0 && (
                <div className="mt-2">
                  <Button
                    ...
                    aria-controls="import-results-errors-list"
          ...
          <div className="flex justify-end gap-3">
            ...
            {importPreview && importPreview.valid && (
              <>
                <Button
                  onClick={() => handleImportCollections(false)}
                ...
                <Button
                  onClick={() => handleImportCollections(true)}
                  disabled={isLoading('importing')}
                  variant="success"
                  isLoading={isLoading('importing')}
                  loadingText="Queuing..."
